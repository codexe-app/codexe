...
import { Stack, ActionIcon, NumberInput, Paper, TextInput, Grid, Button, Group, Popover, Select, Fieldset } from '@mantine/core'
    ...
    <Grid gap={0}>
      <Grid.Col span='content'>
        <Stack className={classes.action}>
          <Popover width={300} opened={addnodem} trapFocus position='bottom' withArrow shadow='md'>
            <Popover.Target>
            ...
            <Popover.Dropdown>
              <form onSubmit={addnode.onSubmit((values) => onAdd(values))}>
                <Stack>
                  ...
                  <Select label='Type' placeholder='Pick value' size='xs' data={nodetypes} defaultValue='default' {...addnode.getInputProps('type')} />
      ...
      <Grid.Col span='content'>
        <Stack className={classes.data}>
          <form onSubmit={form.onSubmit((values) => updateView(values))}>
